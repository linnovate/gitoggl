...
function inject() {
  ...
  chrome.runtime.sendMessage({type: 'activate'}, function(data) {
    ...
    var clientName = document.querySelector('.project-item-select-holder').parentElement.childNodes[0].innerText;

    api_token = data.user.api_token;
    client = data.user.clients.find(client => client.name == clientName);
    project = data.user.projects.find(project => project.name == projectName && project.cid == client.id);
    if(!project) return;
